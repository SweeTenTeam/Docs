...
Verranno effettuati le seguenti tipologie di test:
...
- Test di *accettazione*: la fase finale del "testing". Viene accertato il raggiungimento dei requisiti utente.
...
- *Esito*: "risultato" del test, possono essere 3:
  ...
  - NE (NON ESEGUITO): non è stato "preparato" alcun test per la verifica della funzionalità specifica.

== Test di unità
I test di unità sono il punto di partenza della strategia di testing. Vengono verificate singole unità di codice con l'obbiettivo di verificare che ciascun modulo funzioni correttamente, in maniera tale che ogni unità produca risultati corretti in base ai dati di input inviati. 
...
L'obbiettivo principale di questi test è assicurare che i dati scambiati tra le componenti siano conformi alle specifiche e che i flussi di controllo funzionino regolarmente.
...
== Test di sistema
I test di sistema vengono eseguiti dopo il completamento dei Test di integraxzione e precedono il collaudo.
Si occupano di verificare l'intero sistema come unità, valutando la conformità rispetto ai requisiti presenti nel documento "Analisi dei Requisiti". L'obbiettivo è quello di identificare eventuali errori (che compromettono il corretto funzionamento del sistema) e garantire che il SW soddisfi le aspettative dell'utente.
...
Corrisponde con l'ultima fase della strategia di testing, verificano e accertano il soddisfacimento dei requisiti utente (requisiti del capitolato d'appalto). Questa fase di collaudo viene effettuata alla presenza del committente. 
